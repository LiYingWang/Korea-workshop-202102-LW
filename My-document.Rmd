---
title: "Introduction to spatial data visulization and analysis Using R"
author: "Liying Wang"
date: "2/22/2021"
...
The raster data is DEM data downloaded from https://asterweb.jpl.nasa.gov/gdem.asp. We want to crop the area that matches the site map from this DEM data. Here, we use coordinates to create a data frame, convert it to a spatial object, and then use it to crop the raster.
...
DEM_world <- raster("data/GDEM-10km-colorized.tif")

# assign coordinate reference system
crs(DEM_world) <- "+proj=lcc +lat_1=41.03333333333333 +lat_2=40.66666666666666 +lat_0=40.16666666666666 +lon_0=-74 +x_0=300000 +y_0=0 +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=us-ft +no_defs"

plot(DEM_world) # take a look at raster data
...
crs(sps) <- crs(DEM_world) # define coordinate reference system
crop_DEM <- crop(DEM_world, sps) # crop a raster to the extent of specified spatial object
plot(crop_DEM)
summary(crop_DEM)
```
...
# plot
ggplot() +
  geom_raster(data = crop_DEM_df , aes(x = x, y = y, fill = GDEM.10km.colorized)) +
...
crop_DEM_df %>% 
  ggplot() + 
  geom_raster(aes(x = x, y = y, fill = GDEM.10km.colorized)) +
...
We are curious about the elevation of the locations of archaeological sites, and would like to compare sites from two phases: early neolithic and late neolithic
